...
class FrankaReplayerMoveIt:
    ...
    def _send_gripper_step(self, step_idx, current_state):
        ...
        if current_state == 0:  # closing / grasping
            ...
            rospy.loginfo(f"[{step_idx}] Gripper: GRASPING  (width={target_width:.4f} m)")
        else:                   # opening
            self.move_client.send_goal(MoveGoal(width=target_width, speed=GRIPPER_SPEED))
            rospy.loginfo(f"[{step_idx}] Gripper: OPENING   (width={target_width:.4f} m)")
    ...
    def run(self):
        ...
        self.arm.execute(trajectory, wait=False)  # non-blocking so gripper loop runs
